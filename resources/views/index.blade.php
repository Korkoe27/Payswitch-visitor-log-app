    ...
    <div id="keys-table" class="w-full hidden sm:rounded-lg m-4">
        <table class="w-full text-sm text-left text-gray-500">
            ...
            <tbody class="text-base">
                {{-- @foreach ($keys as $key)
                    <tr class="odd:bg-white even:bg-gray-50 border-b">
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ $key['staff'] }}</th>
                        <td class="px-6 py-4 uppercase">{{ $key['department'] }}</td>
                        <td class="px-6 py-4">{{ $key['time in'] }}</td>
                        <td class="px-6 py-4">
